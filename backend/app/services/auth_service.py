...
def _normalize_datetime(dt: datetime) -> datetime:
    """Return a naive UTC datetime for consistent comparisons."""
...
def login(payload: LoginRequest) -> Tuple[str, Dict[str, object]]:
    ...
    if admin:
        ...
        normalized_expiry = _normalize_datetime(expiry_date) if isinstance(expiry_date, datetime) else expiry_date
        now = datetime.utcnow()

        if isinstance(normalized_expiry, datetime) and now > normalized_expiry:
            days_expired = (now - normalized_expiry).days
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=f"Account expired {days_expired} days ago on {normalized_expiry.strftime('%Y-%m-%d')}",
            )
